...
DiscardFromArray() {
	...
	for i in "${RefArray[@]}"; do
		...
		for ((j = 0; j < $UnsetArraySize; j++)); do
			if [[ "$i" == "${UnsetArray[$j]}" ]]; then
				unset UnsetArray[$j]
				UnsetArray=($(
					IFS=""
					sort <<<${UnsetArray[@]}
				))
			elif [[ "$i" != "${UnsetArray[$j]}" ]]; then
				continue
			fi
		done
	done
...
TempArrayWithAmpersandHasHaveTexts() {
	...
	if [[ $TempArraySize -eq 1 ]]; then
		disk="disk"
		have="has"
	elif [[ $TempArraySize -gt 1 ]]; then
		disk="disks"
		have="have"
	fi
	echo "$disk $have"
...
IsArrayEmpty() {
	...
	local TempArray=("${!TempArrayArgs}")
	unset TempArrayArgs
	[[ -z "${TempArray[@]}" ]] && return 1
	[[ -n "${TempArray[@]}" ]] && return 0
}
...
iw_reconnect() {
	...
	if [[ "${1}" -eq 1 ]]; then
		...
		while read -s -n1 -p "line: " line; do
			if [[ $line == "y" ]]; then
				if [[ -z "${4}" ]]; then
...
iwd_mngr() {
	...
	if [[ ${#wireless_devs[@]} -eq 1 ]]; then
		wireless_card="${wireless_devs[@]}"
	else
		wireless_cards=()
		for i in "${wireless_devs[@]}"; do
			wireless_cards+=("$i")
			wireless_cards+=("")
		done
...
ConfirmMounts() {
	...
	for k in ${m_MountDisksTemp[@]}; do
		...
		local m_partitions=()
		for m in ${m_MountPartitionsTextsTemp[@]}; do
			[[ "$m" =~ "$k" ]] && m_partitions+=("$m")
		done
		...
		if [[ ${#m_partitions[@]} -eq 1 ]]; then
			...
			if [[ -n $partfsformat ]] || [[ "$partfsformat" != "" ]] || [[ ! "$partfsformat" =~ " " ]]; then
				if [[ "$m_parttypename" == "EFI System" ]] || [[ "$m_parttypename" == "EFI (FAT-12/16/32)" ]]; then
					...
					Reformat_Disks+=("${m_partitions[0]}")
				elif [[ "$m_parttypename" == "Linux filesystem" ]] || [[ "$m_parttypename" == "Linux" ]]; then
					if [[ "$partfsformat" == "$linuxfs" ]]; then
						MountPartsString+=" --> *$partfsformat"
					elif [[ "$partfsformat" != "$linuxfs" ]]; then
					...
					Reformat_Disks+=("${m_partitions[0]}")
				elif [[ "$m_parttypename" == "Linux home" ]]; then
					if [[ "$partfsformat" == "$linuxfs" ]]; then
						MountPartsString+=" --> *$partfsformat"
					elif [[ "$partfsformat" != "$linuxfs" ]]; then
						MountPartsString+=" --> *($partfsformat -> $linuxfs)"
			...
			elif [[ -z $partfsformat ]] || [[ "$partfsformat" == "" ]] || [[ "$partfsformat" =~ " " ]]; then
				if [[ "$m_parttypename" == "EFI System" ]] || [[ "$m_parttypename" == "EFI (FAT-12/16/32)" ]]; then
		...
		elif [[ ${#m_partitions[@]} -gt 1 ]]; then
			for l in ${m_partitions[@]}; do
				...
				if [[ "$l" != "${m_partitions[-1]}" ]]; then
					MountPartsString+="  |-/dev/$l --> "
				elif [[ "$l" == "${m_partitions[-1]}" ]]; then
					MountPartsString+="  \`-/dev/$l --> "
				fi
				MountPartsString+="$partsize --> $m_partlabel --> $m_parttypename"

				if [[ -z $partfsformat ]] || [[ "$partfsformat" == "" ]] || [[ $partfsformat =~ " " ]]; then
					if [[ "$m_parttypename" == "EFI System" ]] || [[ "$m_parttypename" == "EFI (FAT-12/16/32)" ]]; then
						MountPartsString+=" --> +FAT32 --> /mnt/boot\n"
						Format_Disks+=("$l")
					elif [[ "$m_parttypename" == "Linux swap" ]]; then
				...
				elif [[ -n $partfsformat ]] || [[ "$partfsformat" != "" ]] || [[ ! $partfsformat =~ " " ]]; then
					if [[ "$m_parttypename" == "EFI System" ]] || [[ "$m_parttypename" == "EFI (FAT-12/16/32)" ]]; then
						MountPartsString+=" --> *FAT32 --> "
						if [[ -z $m_MountPoint ]] || [[ "$m_MountPoint" == "" ]] || [[ "$m_MountPoint" =~ " " ]]; then
	...
	if [[ -z ${Reformat_Disks[@]} ]] && ([[ "${Format_Disks[@]}" == "${m_MountPartitionsTextsTemp[@]}" ]] || [[ ${#Format_Disks[@]} -eq ${#m_MountPartitionsTextsTemp[@]} ]]); then
	...
	elif [[ -n ${Reformat_Disks[@]} ]] && [[ -n ${Format_Disks[@]} ]] && ([[ "${Reformat_Disks[@]}" != "${Format_Disks[@]}" ]] || [[ ${#Reformat_Disks[@]} -ne ${#Format_Disks[@]} ]]); then
	...
	elif [[ -z ${Format_Disks[@]} ]] && ([[ "${Reformat_Disks[@]}" == "${m_MountPartitionsTextsTemp[@]}" ]] || [[ ${#Reformat_Disks[@]} -eq ${#m_MountPartitionsTextsTemp[@]} ]]) && ([[ -z ${m_MountedPartitions[@]} ]] || [[ "${m_MountedPartitions[@]}" != "${m_MountPartitionsTextsTemp[@]}" ]] || [[ ${#m_MountedPartitions[@]} -ne ${#m_MountPartitionsTextsTemp[@]} ]]); then
	...
	elif [[ -z ${Format_Disks[@]} ]] && ([[ "${Reformat_Disks[@]}" == "${m_MountPartitionsTextsTemp[@]}" ]] || [[ ${#Reformat_Disks[@]} -eq ${#m_MountPartitionsTextsTemp[@]} ]]) && ([[ -n ${m_MountedPartitions[@]} ]] || [[ "${m_MountedPartitions[@]}" == "${m_MountPartitionsTextsTemp[@]}" ]] || [[ ${#m_MountedPartitions[@]} -eq ${#m_MountPartitionsTextsTemp[@]} ]]); then
...
FormatPartition() {
	...
	case $m_parttypename in
		"Linux filesystem" | "Linux home" | "Linux")
			if [[ -z $m_existingfs ]] || [[ "$m_existingfs" == "" ]] || [[ "$m_existingfs" =~ " " ]]; then
				mkfs.$fsformat "/dev/$Partition" &>/dev/null | GuageMeter "Formatting partition /dev/$Partition with $fsformat" 1
			elif [[ -n $m_existingfs ]] || [[ "$m_existingfs" != "" ]] || [[ ! "$m_existingfs" =~ " " ]]; then
...
MountPartitions() {
	...
	# fresh mount
	if [[ -n ${mounted[@]} ]] && [[ -z ${fail_drive[@]} ]] && [[ -z ${already_mounted[@]} ]] && [[ -z ${mountfails[@]} ]] && [[ -n ${enabled_swap[@]} ]] && [[ -z ${disabled_swap[@]} ]] && [[ -z ${swap_already_enabled[@]} ]]; then
		mountdialogstring="partitions mounted:\n${mounted[*]}${enabled_swap[*]}"

	# all already mounted
	elif [[ -z ${mounted[@]} ]] && [[ -z ${fail_drive[@]} ]] && [[ -n ${already_mounted[@]} ]] && [[ -z ${mountfails[@]} ]] && [[ -z ${enabled_swap[@]} ]] && [[ -z ${disabled_swap[@]} ]] && [[ -n ${swap_already_enabled[@]} ]]; then
		mountdialogstring="partitions already mounted:\n${already_mounted[*]}${swap_already_enabled[*]}"

	# some already mounted with fails
	elif [[ -n ${mounted[@]} ]] && [[ -n ${fail_drive[@]} ]] && [[ -n ${already_mounted[@]} ]] && [[ -n ${mountfails[@]} ]] && [[ -n ${enabled_swap[@]} ]] && [[ -n ${disabled_swap[@]} ]] && [[ -n ${swap_already_enabled[@]} ]]; then
		mountdialogstring="partitions mounted:\n${mounted[*]}${enabled_swap[*]}\npartitions already mounted:\n${already_mounted[*]}${swap_already_enabled[*]}\npartitions failed to mount:\n${fail_drive[*]}${disabled_swap[*]}"

	# some already mounted without fails
	elif [[ -n ${mounted[@]} ]] && [[ -z ${fail_drive[@]} ]] && [[ -n ${already_mounted[@]} ]] && [[ -n ${mountfails[@]} ]] && [[ -n ${enabled_swap[@]} ]] && [[ -z ${disabled_swap[@]} ]] && [[ -n ${swap_already_enabled[@]} ]]; then
		mountdialogstring="partitions mounted:\n${mounted[*]}${enabled_swap[*]}\npartitions already mounted:\n${already_mounted[*]}${swap_already_enabled[*]}"
	fi
...
CheckEditMount() {
	...
	case $DISKS_WITHOUT_PARTITIONS_PRESENT_EXIT_CODE in
		...
		1)
			...
			if [[ "${m_NoPartsDisks[@]}" == "${m_Disks[@]}" ]]; then
			...
			elif [[ "${m_NoPartsDisks[@]}" != "${m_Disks[@]}" ]]; then
...
IsPartitionTablePresent() {
	...
	[[ "$m_parttable" == "none" ]] && return 1
	[[ "$m_parttable" != "none" ]] && return 0
}
...
EditDisk() {
	...
	for i in "gdisk" "cgdisk" "fdisk" "sfdisk" "cfdisk" "parted"; do
		which "$i" &>/dev/null
		case $? in
			0) diskeditors+=("$i" "$i") ;;
			1) continue ;;
			...
			case $? in
				0)
					...
					for i in "${m_Disks[@]}"; do
						...
						if [[ -n "${m_DiskPartCheck[@]}" ]]; then
							m_NonePartDisks+=("$i")
						elif [[ -z "${m_DiskPartCheck[@]}" ]]; then
							PartProbeDisks+=("$i")
						fi
	...
	case $? in
		0)
			...
			for b in ${m_Disks[@]}; do
				...
				if [[ -n $pttype ]] || [[ "$pttype" != "" ]] || [[ ! "$pttype" =~ " " ]]; then
					UnMountPartitions
				...
				elif [[ -z $pttype ]] || [[ "$pttype" == "" ]] || [[ "$pttype" =~ " " ]]; then
...
PartitionDisk() {
	...
	for i in ${DiskList[@]}; do
		if [[ -z ${DiskPartTable[$i]} ]] && [[ -z ${DiskSize[$i]} ]] && [[ -z ${DiskName[$i]} ]]; then
			continue
		else
			DiskListInfo+=("$i")
	...
	if [[ -n ${m_diskswithparttable[@]} ]]; then
		local REWRITE_PTTYPE_EXIT_CODE
		if [[ ${#m_diskswithparttable[@]} -eq 1 ]]; then
		...
		elif [[ ${#m_diskswithparttable} -gt 1 ]]; then
		...
		0)
			if [[ -z "${Disks[@]}" ]]; then
				...
				if [[ -n "${m_NoPartTableDisks[@]}" ]]; then
					...
					elif [[ "${m_NoPartTableDisks[@]}" != "${Disks[@]}" ]]; then
...
MountViewPartitions() {
	...
	for ((a = 0; a < ${#Disks[@]}; a++)); do
		...
		if [[ -z ${DiskPartSizeTemp[@]} ]] && [[ -z ${DiskPartFsTypeTemp[@]} ]] && [[ -z ${DiskPartName[@]} ]]; then
			# do not consider partitions not consisting any partition info
			continue
		elif [[ -n ${DiskPartSizeTemp[@]} ]] && [[ -n ${DiskPartFsTypeTemp[@]} ]] && [[ -n ${DiskPartName[@]} ]]; then
			# array of partitions
			...
			for i in ${DiskPartName[@]}; do
				...
				if [[ -z $m_fstype ]] || [[ $m_fstype =~ " " ]]; then
					PartFs["$i"]="(Will be formatted)"
				elif [[ -n $m_fstype ]] && [[ ! $m_fstype =~ " " ]]; then
					PartFs["$i"]="$m_fstype"
				fi
				unset m_fstype
			done
			...
			for g in ${partition[@]}; do
				...
				if [[ "$fstype" == "EFI System" || "$fstype" == "EFI (FAT-12/16/32)" ]]; then
					efi_parts+=("$g")
				elif [[ "$fstype" == "Linux filesystem" || "$fstype" == "Linux" ]]; then
					linux_fs_parts+=("$g")
				elif [[ "$fstype" == "Linux swap" ]]; then
					linux_swap_parts+=("$g")
				elif [[ "$fstype" == "Linux home" ]]; then
					linux_home_parts+=("$g")
				elif [[ "$fstype" == "Linux user's home" ]]; then
	...
	elif [[ ${#efi_parts[@]} -eq 1 ]] && [[ ${#linux_fs_parts[@]} -eq 1 ]] && [[ -z ${linux_swap_parts[@]} ]] && [[ -n ${linux_home_parts[@]} ]]; then
	...
	elif [[ ${#efi_parts[@]} -eq 1 ]] && [[ ${#linux_fs_parts[@]} -eq 1 ]] && [[ -z ${linux_home_parts[@]} ]] && [[ -z ${linux_swap_parts[@]} ]]; then
	...
	elif [[ ${#linux_swap_parts[@]} -ge 1 ]] && [[ ${#efi_parts[@]} -eq 1 ]] && [[ ${#linux_fs_parts[@]} -eq 1 ]] && [[ -z ${linux_home_parts[@]} ]]; then
	...
	elif [[ ${#linux_swap_parts[@]} -ge 1 ]] && [[ ${#efi_parts[@]} -eq 1 ]] && [[ ${#linux_fs_parts[@]} -eq 1 ]] && [[ -n ${linux_home_parts[@]} ]]; then
...
Install_UI() {
	...
	case $? in
		...
		0)
			...
			"Desktop Environment")
				...
				case $? in
					...
					0)
						ui_type="$DE"
						case $DE in
							"KDE") pkgs="sddm sddm-kcm $(pacman -Sg plasma kde-{applications,system,graphics,network,accessibility} kf{5,5-aids} | awk '{print $2}' | uniq)" ;;
							"Gnome") pkgs="gnome gnome-extra" ;;
							"cinnamon") pkgs="lightdm $(pacman -Ssq cinnamon)" ;;
							"deepin") pkgs="sddm sddm-kcm deepin deepin-extra" ;;
							"lxde") pkgs="lightdm lxde lxde-gtk3" ;;
							"lxqt") pkgs="lightdm lxqt" ;;
							"mate") pkgs="lightdm mate mate-extra" ;;
							"xfce4") pkgs="lightdm xfce4" ;;
						esac
						;;
				esac
				;;
			...
			if [[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (mountpoint /mnt &>/dev/null) && [[ -d /mnt/boot ]] && (mountpoint /mnt/boot &>/dev/null); then
				pacstrap /mnt $pkgs
	...
	case $? in
		...
		0)
			if [[ -z $NewPassword ]]; then
				...
				if [[ ${#NewPassword} -lt 8 ]] && ([[ -z $NewPassword ]] || [[ "$NewPassword" == "" ]]); then
					dialog --msgbox "password need to be atleast 8 characters long" 0 0
					SetPassword $username
				elif [[ ${#NewPassword} -ge 8 ]]; then
	...
	# set bashrc file to selected users
		...
		case $? in
			0)
				...
				local userText=""
				if [[ ${#SelectedUsers[@]} -eq 1 ]]; then
					userText="user"
				elif [[ ${#SelectedUsers[@]} -gt 1 ]] && [[ ${#SelectedUsers[@]} -lt ${#Users[@]} ]]; then
					userText="users"
				elif [[ ${#SelectedUsers[@]} -eq ${#Users[@]} ]]; then
					userText="all"
				fi

				local SelectedUsersTemp=($(TempArrayWithAmpersand SelectedUsers))

				for i in ${SelectedUsers[@]}; do
...
SetBashPrompt() {
	...
	case $? in
		0)
			SetBashPrompt "$bashrc"
			case $bashrc in
				'default') SetPrompt default_bashrc ;;
				'modded parrot') SetPrompt modded_parrot_bashrc ;;
				'parrot') SetPrompt parrot_bashrc ;;
				'pop OS') SetPrompt bashrc_pop ;;
			esac
			;;
		...
		3)
			...
			case $bashrc in
				'default') BashPromptPreview default_bashrc ;;
				'modded parrot') BashPromptPreview modded_parrot_bashrc ;;
				'parrot') BashPromptPreview parrot_bashrc ;;
				'pop OS') BashPromptPreview bashrc_pop ;;
			esac
			SetBashPrompt $bashrc
			;;
	esac
...
RemoveUsers() {
	...
	if [[ ${#AvailableUsersTemp[@]} -eq 1 ]]; then
	...
	elif [[ ${#AvailableUsersTemp[@]} -gt 1 ]]; then
		...
		case $? in
			...
			0)
				# when base is installed and script is running from live disk
				if [[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (mountpoint /mnt &>/dev/null) && [[ -d /mnt/boot ]] && (mountpoint /mnt/boot &>/dev/null); then
					if [[ ${#UsersToDelete[@]} -ge 1 ]]; then
						for n in ${UsersToDelete[@]}; do
						...
						if [[ ${#DeletedUsers[@]} -eq 1 ]]; then
							UserDeletedString="user"
						elif [[ ${#DeletedUsers[@]} -gt 1 ]]; then
							UserDeletedString="users"
						fi

						if [[ ${#UsersFailedToBeDeleted[@]} -eq 1 ]]; then
							UsersFailedString="user"
						elif [[ ${#UsersFailedToBeDeleted[@]} -gt 1 ]]; then
							UsersFailedString="users"
						fi

						local userDeletedText=""
						if [[ ${#DeletedUsers[@]} -ge 1 ]] && [[ ${#UsersFailedToBeDeleted[@]} -eq 0 ]]; then
						...
						elif [[ ${#DeletedUsers[@]} -eq 0 ]] && [[ ${#UsersFailedToBeDeleted[@]} -ge 1 ]]; then
							userDeletedText="Failed to delete all users"
						elif [[ ${#DeletedUsers[@]} -eq 0 ]] && [[ ${#UsersFailedToBeDeleted[@]} -eq 0 ]]; then
							local DeletedUsersTempTxt=("${DeletedUsers[*]}")
							DeletedUsersTempTxt=("$(TempArrayWithAmpersand DeletedUsersTempTxt)")
				...
				# when base is installed and script is running from installed device
					...
					if [[ ${#UsersToDelete[@]} -ge 1 ]]; then
						...
						if [[ ${#DeletedUsers[@]} -ge 1 ]] && [[ ${#UsersFailedToBeDeleted[@]} -eq 0 ]]; then
							...
							userDeletedText="Failed to delete all users"
						elif [[ ${DeletedUsers[@]} -ge 1 ]] && [[ ${#UsersFailedToBeDeleted[@]} -ge 1 ]]; then
							local DeletedUsersTempTxt=("${DeletedUsers[*]}")
							DeletedUsersTempTxt=("$(TempArrayWithAmpersand DeletedUsersTempTxt)")
...
ConfHost() {
	...
	[[ ! "$(tty)" =~ "tty" ]] && HostOpt+=("Set Bash Prompt" "File that's used to tell how the terminal prompt should look like")

	# when base is installed and script is running from live disk
	if ( (! mountpoint /mnt &>/dev/null) || ([[ ! -d /mnt/boot ]] && (! mountpoint /mnt/boot &>/dev/null))) && ([[ ! -d /run/archiso/airootfs ]] && [[ ! -d /run/archiso/bootmnt ]]) && ( (mountpoint / &>/dev/null) && (mountpoint /boot &>/dev/null)); then
		usersTemp=($(cat /etc/passwd | sed 's/:/ : /g' | grep -iG '[1-9][0-9][0-9][0-9]\d*' | grep -iv nobody | awk '{ print $1 }'))

	# when base is installed and script is running from installed device
	elif [[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (mountpoint /mnt &>/dev/null) && [[ -d /mnt/boot ]] && (mountpoint /mnt/boot &>/dev/null); then
...
InstallArch() {
	...
	if ([[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (! mountpoint /mnt &>/dev/null) && [[ ! -d /mnt/boot ]] && (! mountpoint /mnt/boot &>/dev/null)); then
		...
		# expanding the regex package variable for display in dialog
		for i in "${extra_linux_kernel_pkgs_temp[@]}"; do
		...
		case $? in
			...
			*)
				...
				elif ([[ ! -d /mnt ]] || (! mountpoint /mnt &>/dev/null)) || ([[ ! -f "/mnt/etc/fstab" ]] && [[ ! -d "/mnt/etc/" ]]); then
					dialog --msgbox "Failed to install Arch Base system. Exiting the Installer" 0 0
				fi
				exit
				;;
...
Repo_Enable() {
	...
	if [[ "$multilib_include_line" != "Inlclude*" ]] && [[ "$multilib_line" != "[multilib]" ]]; then
...
MainMenu() {
	...
	if [[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (mountpoint /mnt &>/dev/null) && [[ -d /mnt/boot ]] && (mountpoint /mnt/boot &>/dev/null) || ([[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (! mountpoint /mnt &>/dev/null) && [[ ! -d /mnt/boot ]]); then
			...
			case $menuitem in
				...
				"Install Arch *")
					if ([[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (mountpoint /mnt &>/dev/null) && [[ -d /mnt/boot ]] && (mountpoint /mnt/boot &>/dev/null)); then
						Repo_Enable
						InstallArch
					elif ([[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null)) && ( (! mountpoint /mnt) || ([[ ! -d /mnt/boot ]] && (! mountpoint /mnt/boot))); then
				...
				"Configure Host +" | "Configure Host")
					...
					elif [[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (! mountpoint /mnt &>/dev/null) && [[ ! -d /mnt/boot ]]; then
...
Main() {
	...
	if ([[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (mountpoint /mnt &>/dev/null) && [[ -d /mnt/boot ]] && (mountpoint /mnt/boot &>/dev/null)) || ([[ -d /run/archiso/airootfs ]] && [[ -d /run/archiso/bootmnt ]] && (mountpoint /run/archiso/airootfs &>/dev/null) && (mountpoint /run/archiso/bootmnt &>/dev/null) && (! mountpoint /mnt &>/dev/null) && [[ ! -d /mnt/boot ]]); then
		...
		case $DIALOG_PRESENT_EXIT_CODE in
			1)
				for i in wget git grub "os-prober"; do
				...
				;;
			0)
				for i in wget git grub "os-prober" "NetworkManager"; do
					pacman -Qqs "$i" &>/dev/null
					case $? in
		...
		if [[ -n ${needed_components_absent[@]} ]]; then
